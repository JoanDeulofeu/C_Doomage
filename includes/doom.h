...
typedef struct				s_walls {
	...
	int						floor_height;
	int						ceiling_height;
	int						floor_height_dest;
	int						ceiling_height_dest;
	int						wall_height_tmp;
	int						nb_tex;
	t_image					*image;
	struct s_walls			*prev;
...
*/
void						ft_multi_threading(t_main *s, t_walls *wall,
							double l_height_wall, double r_height_wall);
...
t_texture					*initialize_texture(t_sdl *sdl, int width,
							int height);
...
void						ft_draw_editor(t_editor *edi,
							t_texture *tex);
...
void						set_pixel(t_texture *text, Uint32 color,
							t_pos coord);
...
t_4double					ft_check_sector_sens2(t_int *wall, t_4double score,
							t_pos center);
...
void						add_portal_ptr(t_main *s, t_int *wall, int i,
							t_sector *sct);
void						ft_norm_parse_sector(t_main *s, char *line,
							t_sector *sct, int i);
...
int							check_between_wall(t_main *s, t_int *wall,
							t_pos mouse);
...
int							check_portal_validity(t_main *s, t_int *vtx,
							t_visu *vs, int gap);
...
void						ft_trace_vertical_select(t_main *s, t_line line,
							Uint32 color);
int							ft_trace_line_select(t_main *s, t_line line,
							Uint32 color);
void						ft_get_line_select(t_main *s, t_line line,
							Uint32 color);
...
void						ft_trace_vertical(t_main *s, t_line line,
							Uint32 color);
void						ft_get_line(t_main *s, t_line line,
							Uint32 color);
int							ft_trace_line(t_main *s, t_line line,
							Uint32 color);
...
void						ft_draw_floor(t_main *s, t_walls *wall,
							t_pos coord);
int							ft_draw_ceiling(t_main *s, t_walls *wall,
							t_pos coord);
void						draw_texture(t_multithread *mt, t_pos coord,
							int end);
void						get_total_w_wall(t_walls *wall);
void						change_wall_texture(t_main *s, int up);
void						ft_draw_column(t_pos coord, int end,
							t_multithread *mt);
...
void						display_sprites(t_main *s, int display, int fad,
							int fai);
...
int							check_limit_sprite(t_main *s, int sct_id,
							t_sprite *sprite);
void						free_sprite(t_main *s);
...
t_sprite					*create_new_sprite(t_main *s, t_name name,
							t_dpos pos, t_sprite *temp);
void						get_sprite_info_by_name(t_main *s, t_name name,
							t_sprite *sprite);
t_sprite					*is_sprite_under_mouse(t_main *s);
int							is_sprite_selected(t_main *s);
void						display_sprite_menu(t_main *s, int i, int j,
							int px);
...
void						draw_plain_sprite(int i, t_pos coord, t_image *img,
							t_texture *tex);
int							check_if_visible(t_main *s, t_sprite *sprite);
...
void						move_vertex(t_main *s, t_pos tmp_mosve, t_pos ori,
							int id);
...
void						rand_move(t_main *s, double angle,
							t_sprite *sprite);
...
void						check_sprite_shooting(t_main *s,
							t_sprite *sprite);
void						get_anim_by_angle(t_main *s, t_sprite *sprite,
							double angle);
void						set_img(t_main *s, t_sprite *cur, int id,
							int orientation);
