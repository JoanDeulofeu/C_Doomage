...
void	ft_place_view_plan(t_main *s, Uint32 color)
{
	...
	center_plan.x = s->player.pos.x + cos(to_rad(s->player.angle)) * SPACEPLAN;
	center_plan.y = s->player.pos.y - sin(to_rad(s->player.angle)) * SPACEPLAN;
...
void	ft_visu_joan(t_main *s)
{
	int		mur1;
	int		mur2;
	Uint32	color = 0xffed00ff;

	s->player.sector = ft_is_in_sector(s, ft_dpos_to_pos(s->player.pos));
	ft_place_view_plan(s, color);
	mur1 = ft_find_wall2(s, s->left_plan, color);
	mur2 = ft_find_wall2(s, s->right_plan, color);
	printf("mur gauche = %d\nmur droite = %d\n\n", mur2, mur1);
