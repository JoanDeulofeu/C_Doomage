...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/06 15:48:48 by ydonse            #+#    #+#             */
/*   Updated: 2020/02/06 15:50:03 by ydonse           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	trace_vertical(t_main *s, Uint32 color)
{
	...
	s->line.e = s->line.y2 > s->line.y1 ? s->line.y2 - s->line.y1
	: s->line.y1 - s->line.y2;
	sens_x = s->line.x2 > s->line.x1 ? 1 : -1;
	sens_y = s->line.y2 > s->line.y1 ? 1 : -1;
	s->line.dy = s->line.e * 2;
	s->line.dx = s->line.x2 > s->line.x1 ? (s->line.x2 - s->line.x1)
	* 2 : (s->line.x1 - s->line.x2) * 2;
...
{
	s->line.e = s->line.x2 > s->line.x1 ? s->line.x2 - s->line.x1 :
	s->line.x1 - s->line.x2;
	s->line.dy = s->line.y2 > s->line.y1 ? (s->line.y2 - s->line.y1) * 2 :
	(s->line.y1 - s->line.y2) * 2;
